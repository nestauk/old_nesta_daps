...
from nesta.packages.mag.query_mag_sparql import (
    update_field_of_study_ids_sparql,
    extract_entity_id,
    query_articles_by_doi,
    query_authors,
)
        ...
        with db_session(self.engine) as session:
            ...
            articles_to_process = [
                ...
                for a in (
                    ...
                    .filter(
                        (Article.mag_authors.is_(None) | ~Article.fields_of_study.any())
            ...
            for count, row in enumerate(
                ...
                try:
                    ...
                    row["fields_of_study"] = {
                        extract_entity_id(f) for f in fos.split(",")
                    }
                except KeyError:
                ...
                try:
                    author_ids = {
                        extract_entity_id(a) for a in row.pop("authors").split(",")
                    }
                ...
                if self.test and count == 3000:
                    logging.warning("Exiting after 3000 rows in test mode")
                    break
