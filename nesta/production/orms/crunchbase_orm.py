...
class Organization(Base):
    ...
    company_name = Column(VARCHAR(200, collation='utf8_bin'))
    roles = Column(VARCHAR(50))
    permalink = Column(VARCHAR(200))
    domain = Column(TEXT)
    homepage_url = Column(TEXT)
    location_id = Column(VARCHAR(400, collation='utf8_bin'), index=True)
    country = Column(VARCHAR(200, collation='utf8_bin'))
    state_code = Column(VARCHAR(2))
    region = Column(VARCHAR(100, collation='utf8_bin'))
    city = Column(VARCHAR(100, collation='utf8_bin'))
    address = Column(VARCHAR(200, collation='utf8_bin'))
    status = Column(VARCHAR(9))
    short_description = Column(VARCHAR(200, collation='utf8_bin'))
    ...
    email = Column(VARCHAR(200, collation='utf8_bin'))
    phone = Column(TEXT)
...
class Acquisition(Base):
    ...
    acquiree_name = Column(VARCHAR(200, collation='utf8_bin'))
    acquiree_country_code = Column(VARCHAR(3))
    state_code = Column(VARCHAR(2))
    acquiree_region = Column(VARCHAR(50, collation='utf8_bin'))
    acquiree_city = Column(VARCHAR(50, collation='utf8_bin'))
    acquirer_name = Column(VARCHAR(200, collation='utf8_bin'))
...
class Fund(Base):
    ...
    fund_name = Column(VARCHAR(200, collation='utf8_bin'))
    announced_on = Column(DATE)
    raised_amount = Column(BIGINT)
    raised_amount_currency_code = Column(VARCHAR(3))
    created_at = Column(DATETIME)
...
class People(Base):
    ...
    id = Column(VARCHAR(36), primary_key=True)
    first_name = Column(VARCHAR(100, collation='utf8_bin'))
    last_name = Column(VARCHAR(100, collation='utf8_bin'))
    location_id = Column(VARCHAR(400, collation='utf8_bin'), index=True)
    country = Column(VARCHAR(200, collation='utf8_bin'))
    state_code = Column(VARCHAR(2))
    city = Column(VARCHAR(100, collation='utf8_bin'))
    ...
    facebook_url = Column(TEXT)
    linkedin_url = Column(TEXT)
    primary_affiliation_organization = Column(VARCHAR(100, collation='utf8_bin'))
    primary_affiliation_title = Column(VARCHAR(100, collation='utf8_bin'))
